...
export default function InventoryManager({ 
  ...
  const filteredItems = useMemo(() => {
    return initialItems.filter((item) => {
      ...
      return matchSearch && matchCondition && matchStatus && matchCategory;
    });
  }, [initialItems, search, filterCondition, filterStatus, filterCategory]);
  ...
  // Componente Auxiliar para mostrar de quem é o Material
  const AllocationBadge = ({ item }: { item: any }) => {
    if (item.patrol) {
      return <span className="bg-amber-50 text-amber-600 px-2 py-1 rounded text-[10px] font-bold border border-amber-100"><i className="fa-solid fa-paw mr-1"></i> {item.patrol.name}</span>;
    }
    if (item.troop) {
      return <span className="bg-scout-green/10 text-scout-green px-2 py-1 rounded text-[10px] font-bold border border-scout-green/20"><i className="fa-solid fa-tent mr-1"></i> {item.troop.name}</span>;
    }
    return <span className="bg-gray-100 text-gray-500 px-2 py-1 rounded text-[10px] font-bold border border-gray-200"><i className="fa-solid fa-globe mr-1"></i> Grupo</span>;
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 overflow-x-auto custom-scrollbar pb-2">
        ...
        >
          <i className="fa-solid fa-boxes-stacked"></i> Estoque de Materiais
        </button>
        <button
        ...
        >
          <i className="fa-solid fa-clock-rotate-left"></i> Histórico de Empréstimos
        </button>
      </div>
          ...
          <div className="flex flex-col xl:flex-row gap-4 justify-between bg-white p-4 md:p-5 rounded-2xl shadow-sm border border-gray-200">
            <div className="flex flex-col md:flex-row flex-1 gap-3 md:gap-4">
              ...
              <div className="relative flex-1 md:max-w-xs">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none"><i className="fa-solid fa-magnifying-glass text-gray-400"></i></div>
                <input type="text" placeholder="Buscar material ou Nº..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-scout-green/20 focus:border-scout-green outline-none transition-all text-sm font-medium" />
              </div>

              <div className="relative flex-1 md:max-w-45">
                <select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)} className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-scout-green/20 focus:border-scout-green outline-none transition-all text-sm font-bold text-gray-700 appearance-none cursor-pointer">
              ...
              <div className="relative flex-1 md:max-w-50">
                <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="w-full pl-10 pr-8 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-scout-green/20 focus:border-scout-green outline-none transition-all text-sm font-bold text-gray-700 appearance-none cursor-pointer">
                  <option value="ACTIVE_ALL">Estoque Ativo</option>
                  <option value="AVAILABLE">Disponíveis</option>
                  <option value="CHECKED_OUT">Emprestados</option>
                  <option value="DISCHARGED">Baixados / Descartados</option>
                </select>
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none"><i className="fa-solid fa-filter text-gray-400"></i></div>
                <div className="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none"><i className="fa-solid fa-chevron-down text-gray-400 text-xs"></i></div>
              </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button onClick={() => setReportModalOpen(true)} className="cursor-pointer bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 px-4 py-2.5 rounded-xl font-medium flex items-center justify-center gap-2 shadow-sm transition-all active:scale-95 whitespace-nowrap">
                <FileText className="w-5 h-5 text-blue-600" /> Relatórios
              </button>
              <button onClick={handleOpenNew} className="cursor-pointer w-full md:w-auto bg-scout-green hover:bg-green-700 text-white px-6 py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 shadow-sm transition-all active:scale-95 shrink-0">
              ...
              filteredItems.map((item) => {
                ...
                return (
                  <div key={item.id} className={`bg-white p-4 rounded-2xl border ${!item.isActive ? 'border-red-200 opacity-80' : 'border-gray-200'} shadow-sm relative`}>
                    ...
                    {/* Status Box e Ações (Mantidos do Original) */}
                    <div className="mb-4">
                      {!item.isActive ? (
                    ...
                    {item.isActive && (
                      <div className="grid grid-cols-2 gap-2 border-t border-gray-100 pt-4">
                        <button onClick={() => handleOpenCheckout(item)} className={`cursor-pointer py-2 rounded-xl text-sm font-bold border transition-colors flex items-center justify-center gap-2 ${activeLoan ? "bg-white border-gray-200 text-gray-700 hover:bg-gray-50" : "bg-blue-50 border-blue-200 text-blue-700 hover:bg-blue-100"}`}>
                          <i className={`cursor-pointer fa-solid ${activeLoan ? 'fa-arrow-rotate-left' : 'fa-hand-holding-hand'}`}></i>
                          {activeLoan ? "Devolver" : "Emprestar"}
                        </button>
                        <div className="flex gap-2">
                          <button onClick={() => handleOpenEdit(item)} className="cursor-pointer flex-1 bg-gray-50 border border-gray-200 text-gray-600 hover:bg-gray-100 py-2 rounded-xl text-sm font-bold flex items-center justify-center transition-colors"><i className="fa-solid fa-pen"></i></button>
                          <button onClick={() => handleOpenDischarge(item)} className="cursor-pointer w-12 bg-red-50 border border-red-200 text-red-500 hover:bg-red-100 hover:text-red-700 py-2 rounded-xl text-sm font-bold flex items-center justify-center transition-colors"><i className="fa-solid fa-trash"></i></button>
                        </div>
                      </div>
                    )}
                  </div>
            ...
            <table className="w-full text-left border-collapse min-w-200">
              <thead>
                <tr className="bg-gray-50 text-gray-500 text-xs font-bold uppercase tracking-widest border-b border-gray-200">
                  ...
                  <th className="p-4">Material & Alocação</th>
                  ...
                  return (
                    <tr key={item.id} className={`hover:bg-gray-50/80 transition-colors group ${!item.isActive ? 'bg-red-50/30' : ''}`}>
                      ...
                      <td className="p-2 align-middle">
                        <span className="font-mono bg-gray-100 border border-gray-200 px-2 py-1 rounded text-xs text-gray-700 font-bold shadow-sm tracking-wider">#{item.chargeNumber}</span>
                      </td>
                      ...
                      <td className="p-4 align-middle">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className={`font-bold text-sm ${item.isActive ? 'text-gray-900' : 'text-gray-500 line-through decoration-red-400'}`}>{item.name}</p>
                            <div className="flex items-center gap-2 mt-1">
                               <p className="text-[10px] text-gray-500 font-medium uppercase">{item.category}</p>
                               <AllocationBadge item={item} />
                            </div>
                          </div>
                        </div>
                      ...
                      <td className="p-4 text-center font-black text-gray-700 align-middle">{item.quantity}</td>
                      ...
                      <td className="p-4 align-middle">
                        {!item.isActive ? (
                           <div className="flex flex-col gap-1 text-red-700 max-w-50">
                             <div className="flex items-center gap-1.5 font-bold text-[10px] uppercase tracking-wider"><i className="fa-solid fa-triangle-exclamation"></i> Baixado</div>
                          ...
                          <div className="flex flex-col gap-1 text-amber-700 max-w-50">
                            <div className="flex items-center gap-1.5 font-bold text-[10px] uppercase tracking-wider"><i className="fa-regular fa-clock"></i> Emprestado para</div>
                      ...
                      <td className="p-4 text-right align-middle">
                        {item.isActive && (
                          <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => handleOpenCheckout(item)} className={`cursor-pointer text-xs px-3 py-2 rounded-lg font-bold transition-colors border ${activeLoan ? "bg-white border-gray-200 text-gray-700 hover:bg-gray-100" : "bg-blue-50 border-blue-200 text-blue-700 hover:bg-blue-100"}`}>
                              {activeLoan ? "Devolver" : "Emprestar"}
                            </button>
                            <button onClick={() => handleOpenEdit(item)} className="cursor-pointer w-8 h-8 bg-gray-50 border border-gray-200 text-gray-600 rounded-lg hover:bg-gray-200 hover:border-gray-300 transition-all flex items-center justify-center" title="Editar"><i className="fa-solid fa-pen text-xs"></i></button>
                            <button onClick={() => handleOpenDischarge(item)} className="cursor-pointer w-8 h-8 bg-white border border-red-200 text-red-500 rounded-lg hover:bg-red-50 hover:text-red-700 transition-all flex items-center justify-center" title="Dar Baixa"><i className="fa-solid fa-trash text-xs"></i></button>
                          </div>
                        )}
                      </td>
                    </tr>
      ...
      {/* Modais */}
      <ItemModal isOpen={isItemModalOpen} onClose={() => setItemModalOpen(false)} item={selectedItem} troops={troops} />
